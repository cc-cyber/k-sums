...
const int XTKSums::nTrails = 1;
const int XTKSums::nIter   = 31;    ///set iteration times
...
int XTKSums::initIm(const unsigned int nclust)
{
...
int XTKSums::initIs(const unsigned int nclust)
{
    unsigned int i = 0, j = 0, c = 0, loc = 0;
    memset(this->arrayD, 0, sizeof(double)*nclust*this->ndim);
    memset(this->labels, 0, sizeof(int)*this->count);
...
double XTKSums::optzIm(const unsigned int nIter, const int nclust)
{
    ...
    for(iter = 0; iter < nIter; iter++)///set nIter to 30 normally
    {
        ...
        for(i = 0; i < this->count; i++)///one round
        {
            ...
            /**/
            distA = l2nDst(this->arrayD+cloc, s, n1+1);
            distA = distA/(n1*n1);
...
double XTKSums::optzIs(const unsigned int nIter, const int nclust)
{
    unsigned int i = 0, iter = 0, j = 0, cloc = 0, sloc = 0, dloc = 0;
...
void XTKSums::test()
{
    ...
    const char *srcFn2     = "";
    const char *distortFn2 = "";
    const char *dstFn2 = "";

    int cnum = 1024;

    XTKSums *mykm = new XTKSums();
    mykm->buildcluster(srcFn1, dstFn1, "rnd", "large", "", cnum, false);
    delete mykm;
